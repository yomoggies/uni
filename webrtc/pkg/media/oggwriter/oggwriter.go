...
/*
    ...
    |            | |   | |   |     |   | |           | |         | |
    |+----------+| |+-----------------+| |+-------------------+ +-----
    |||ConnectionID Header|| ||  Comment Header || ||Audio Data Packet 1| | ...
    |+----------+| |+-----------------+| |+-------------------+ +-----
    |            | |   | |   |     |   | |           | |         | |
    +------------+ +---+ +---+ ... +---+ +-----------+ +---------+ +--
    ^      ^                           ^
    ...
    |      |
    |      ConnectionID header is contained on a single page
    |
    'Beginning Of Stream'
...
func (i *OggWriter) writeHeaders() error {
	...
	// RFC specifies that the ConnectionID Header page should have a granule position of 0 and a Header Type set to 2 (StartOfStream)
