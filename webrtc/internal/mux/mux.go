...
func (m *Mux) pendingPacketHandler() {
	...
	for {
		...
		case <-ticker.C:
			m.lock.RLock()
			m.pendingPacketsLock.Lock()
			for e := range m.pendingPackets {
				if time.Since(e.t) > time.Second*5 {
					m.log.Warnf("Warning: mux: dropping packet after 5 seconds in pending queue")
					delete(m.pendingPackets, e)
				}
				for endpoint, f := range m.endpoints {
					if f(e.data) {
						endpoint.buffer.Write(e.data)
						delete(m.pendingPackets, e)
						m.log.Warnf("Warning: mux: found endpoint for packet after %s in pending queue", time.Since(e.t))
					}
				}
			}
			m.pendingPacketsLock.Unlock()
